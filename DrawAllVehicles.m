...
function vehicle = DrawAllVehicles(inter, vehicle, int, road, lane, time, max_speed)
    ...
    if isnan(road) == false
        ...
        for j = 1:num_spawned
            [vehicle] = MakeVehicle(inter, vehicle, in_queue + j, int(j), road(j), lane(j), time, max_speed);
